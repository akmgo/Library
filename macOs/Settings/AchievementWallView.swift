...
struct BadgeDef: Identifiable {
    let id: Int; let title: String; let description: String; let icon: String; let rarity: BadgeRarity
    let conditionCheck: (AchievementData) -> Date?
}

struct AchievementData {
    let books: [Book]; let records: [ReadingRecord]; let excerpts: [Excerpt]; let notes: [Note]
}

// MARK: - 2. 核心计算引擎 (50个硬核成就完整逻辑)
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 0) {
                ...
                // ✨ 优化 1：使用单一 LazyVGrid 配合 Section，彻底解决滚动高度计算导致的位置乱跳
...
struct MasterpieceBadgeView: View {
    ...
    var body: some View {
        // ✨ 优化 2：绝对刚性容器。提供一个死死固定为 100x140 的外部框架。
            ...
            VStack(spacing: 10) {
                ...
                ZStack {
                    if isUnlocked && (badge.rarity == .obsidian || badge.rarity == .diamond) {
                        // ✨ 优化 3：用 RadialGradient 替代极度耗费性能的 .blur(radius: 8)
                        Circle()
