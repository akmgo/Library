    ...
    @MainActor
        private func fetchRealData() async -> FocusHeroEntry {
            ...
            do {
                ...
                // ✨ 核心修复 1：优先使用 lastReadAt (最后阅读时间)
                // 确保小组件永远跟着你的“视线”走，而不是跟着“新建时间”走
                ...
                if let book = mostRecentBook {
                    ...
                    // ✨ 核心修复 2：加上 Predicate 日期过滤条件
                    // 绝不全库拉取，只让 SQLite 把今天及以后的记录返回给我们，彻底排雷内存炸弹
                    let recordDescriptor = FetchDescriptor<ReadingSession>(
                        predicate: #Predicate<ReadingSession> { $0.date >= today }
                    )
                    ...
                    if let todayRecord = records.first(where: { Calendar.current.isDate($0.date, inSameDayAs: today) }) {
                        todayMins = Int(todayRecord.duration / 60)
                    }
                    ...
                    return FocusHeroEntry(
                        ...
                        bookProgress: book.progressRatio,
