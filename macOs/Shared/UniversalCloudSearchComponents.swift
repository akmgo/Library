...
struct UniversalSearchResultRow: View {
    let result: BookSearchResult
    let mode: SearchRowMode
    ...
    var body: some View {
        ...
        HStack(alignment: .center, spacing: isSpotlight ? 16 : 12) {
            ...
            .task(id: result.coverURL) {
                ...
                isLoadingCover = true
                coverData = await CloudSearchManager.shared.fetchCoverData(from: url)
...
struct InlineSearchResultsPopover: View {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    LazyVStack(spacing: 4) {
                        ForEach(results) { res in
                            ...
                            UniversalSearchResultRow(result: res, mode: .popover, isImporting: false) { fetchedCover in
